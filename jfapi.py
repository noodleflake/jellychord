...
class Track:
    def __init__(self):
        # empty track with no data
        self.__init__('', '', 0, [])
    
    def __init__(self, id: str, title: str, length: int, artists: list[str] | None = []):
        self._artists = artists
        self._title = title
        self._length = length
        self._id = id
    
    def __init__(self, queryResult: dict):
        self.__init__(
            queryResult['Id'],
            queryResult['Name'],
            queryResult['RunTimeTicks'] // 10000000,
            queryResult['Artists']
        )
